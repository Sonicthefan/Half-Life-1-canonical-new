...
#define NUM_SCIENTIST_HEADSHL2 3 // three heads available for scientist model
enum
{
	HEAD_VANCE = 0,
	HEAD_KLEINER = 1,
...
//=========================================================
#define SCIENTIST_AE_HEAL_HL2 (1)
#define SCIENTIST_AE_NEEDLEON_HL2 (2)
#define SCIENTIST_AE_NEEDLEOFF_HL2 (3)
...
LINK_ENTITY_TO_CLASS(monster_hl2scientist, CScientist2);
...
char* CScientist2::GetScientistModel() const
{
	...
	return "models/unique_scientists.mdl";
}
...
Schedule_t slhl2Follow[] =
	{
		{tlhl2Follow,
			ARRAYSIZE(tlhl2Follow),
			bits_COND_NEW_ENEMY |
...
Schedule_t slhl2FollowScared[] =
	{
		{tlhl2FollowScared,
			ARRAYSIZE(tlhl2FollowScared),
			bits_COND_NEW_ENEMY |
				bits_COND_HEAR_SOUND |
				bits_COND_LIGHT_DAMAGE |
				bits_COND_HEAVY_DAMAGE,
...
Schedule_t slhl2FaceTargetScared[] =
	{
		{tlhl2FaceTargetScared,
			ARRAYSIZE(tlhl2FaceTargetScared),
			bits_COND_HEAR_SOUND |
				bits_COND_NEW_ENEMY,
			bits_SOUND_DANGER,
			"FaceTargetScared"},
...
Task_t tlhl2StopFollowing[] =
	{
		{TASK_CANT_FOLLOW, (float)0},
};

Schedule_t slhl2StopFollowing[] =
	{
		{tlhl2StopFollowing,
			ARRAYSIZE(tlhl2StopFollowing),
			0,
			0,
			"StopFollowing"},
};
...
Schedule_t slhl2FaceTarget[] =
	{
		{tlhl2FaceTarget,
			ARRAYSIZE(tlhl2FaceTarget),
...
Schedule_t slhl2SciPanic[] =
	{
		{tlhl2SciPanic,
			ARRAYSIZE(tlhl2SciPanic),
			0,
			0,
			"SciPanic"},
};
...
Schedule_t slhl2IdleSciStand[] =
	{
		{tlhl2IdleSciStand,
			ARRAYSIZE(tlhl2IdleSciStand),
			bits_COND_NEW_ENEMY |
				bits_COND_LIGHT_DAMAGE |
				bits_COND_HEAVY_DAMAGE |
				bits_COND_HEAR_SOUND |
...
Schedule_t slhl2ScientistCover[] =
	{
		{tlhl2ScientistCover,
			ARRAYSIZE(tlhl2ScientistCover),
			bits_COND_NEW_ENEMY,
			0,
			"ScientistCover"},
};
...
Schedule_t slhl2ScientistHide[] =
	{
		{tlhl2ScientistHide,
			ARRAYSIZE(tlhl2ScientistHide),
			bits_COND_NEW_ENEMY |
				bits_COND_HEAR_SOUND |
				bits_COND_SEE_ENEMY |
				bits_COND_SEE_HATE |
...
Schedule_t slhl2ScientistStartle[] =
	{
		{tlhl2ScientistStartle,
			ARRAYSIZE(tlhl2ScientistStartle),
			bits_COND_NEW_ENEMY |
				bits_COND_SEE_ENEMY |
				bits_COND_SEE_HATE |
				bits_COND_SEE_FEAR |
...
DEFINE_CUSTOM_SCHEDULES(CScientist2){
	...
	slhl2ScientistCover,
	slhl2ScientistHide,
	slhl2ScientistStartle,
	slhl2Heal,
	slhl2StopFollowing,
	slhl2SciPanic,
	slhl2FollowScared,
	slhl2FaceTargetScared,
};
...
void CScientist2::DeclineFollowing()
{
	Talk(10);
...
Activity CScientist2::GetStoppedActivity()
{
	if (m_hEnemy != NULL)
		return ACT_EXCITED;
...
void CScientist2::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	switch (pEvent->event)
	{
	case SCIENTIST_AE_HEAL_HL2: // Heal my target (if within range)
		Heal();
		break;
	case SCIENTIST_AE_NEEDLEON_HL2:
	{
		int oldBody = pev->body;
		pev->body = (oldBody % NUM_SCIENTIST_HEADSHL2) + NUM_SCIENTIST_HEADSHL2 * 1;
	}
	break;
	case SCIENTIST_AE_NEEDLEOFF_HL2:
	{
		int oldBody = pev->body;
		pev->body = (oldBody % NUM_SCIENTIST_HEADSHL2) + NUM_SCIENTIST_HEADSHL2 * 0;
	}
	break;
...
// Spawn
//=========================================================
void CScientist2::Spawn()
{
	if (pev->body == -1)
	{														 // -1 chooses a random head
		pev->body = RANDOM_LONG(0, NUM_SCIENTIST_HEADSHL2 - 1); // pick a head, any head
	}
	...
	// Eli is black, make his hands black
	if (pev->body == HEAD_VANCE)
		pev->skin = 1;
...
void CScientist2::TalkInit()
{
	...
	// get voice for head
	switch (pev->body % NUM_SCIENTIST_HEADSHL2)
	{
	default:
	case HEAD_KLEINER:
		m_voicePitch = 105;
...
bool CScientist2::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
...
Schedule_t* CScientist2::GetScheduleOfType(int Type)
{
	...
	case SCHED_TARGET_FACE:
		...
		if (psched == slIdleStand)
			return slhl2FaceTarget; // override this for different target face behavior
		else
			return psched;
	...
	case SCHED_CANT_FOLLOW:
		return slhl2StopFollowing;

	case SCHED_PANIC:
		return slhl2SciPanic;

	case SCHED_TARGET_CHASE_SCARED:
		return slhl2FollowScared;

	case SCHED_TARGET_FACE_SCARED:
		return slhl2FaceTargetScared;
	...
	case SCHED_IDLE_STAND:
		...
		if (psched == slIdleStand)
			return slhl2IdleSciStand;
		else
			return psched;

	case SCHED_HIDE:
		return slhl2ScientistHide;

	case SCHED_STARTLE:
		return slhl2ScientistStartle;
...
Schedule_t* CScientist2::GetSchedule()
{
	...
	case MONSTERSTATE_COMBAT:
		if (HasConditions(bits_COND_NEW_ENEMY))
			return slhl2Fear; // Point and scream!

		if (HasConditions(bits_COND_SEE_ENEMY))
		{
			// Marphy Fact Files Fix - Fix scientists not disregarding enemy after hiding
			m_fearTime = gpGlobals->time;
			return slhl2ScientistCover; // Take Cover
		}
		...
		if (pEnemy)
		{
			...
			else if (DisregardEnemy(pEnemy)) // After 15 seconds of being hidden, return to alert
			{
				...
				return slhl2ScientistHide; // Hide after disregard
			}
		}

		return slhl2ScientistCover; // Run & Cower
		break;
	}
...
MONSTERSTATE CScientist2::GetIdealState()
{
	switch (m_MonsterState)
	{
	case MONSTERSTATE_ALERT:
...
LINK_ENTITY_TO_CLASS(monster_sitting_hl2scientist, CSittingScientist2);
...
void CSittingScientist2::Precache()
{
	m_baseSequence = LookupSequence("sitlookleft");
	TalkInit();
}
...
//=========================================================
int CSittingScientist2::Classify()
{
	return CLASS_HUMAN_PASSIVE;
}
...
// sit, do stuff
//=========================================================
void CSittingScientist2::SittingThink()
{
	CBaseEntity* pent;
...
//=========================================================
bool CSittingScientist2::FIdleSpeak()
{
